...
def extract_info(filename):
    """
    Extrai informações do nome do arquivo no formato EXYZZ
    X = número do experimento
    Y = número da replicata
    ZZ = número da amostra
    """
    basename = os.path.basename(filename)
    match = re.match(r'E(\d)(\d)(\d{2})\.csv', basename)
    if match:
...
def buscar_arquivos_cromatogramas(num_experimento, diretorio_base='.'):
    """
    Busca automaticamente todos os arquivos CSV no formato EXYZZ na pasta EXP X
    """
    ...
    # Buscar todos os arquivos CSV que seguem o padrão EXYZZ dentro da pasta
    pattern = os.path.join(pasta_experimento, f'E{num_experimento}[0-9][0-9][0-9].csv')
    files = glob.glob(pattern)

    if not files:
        print(f"⚠ Nenhum arquivo encontrado no padrão E{num_experimento}YZZ.csv na pasta: {pasta_experimento}")
        return []

    print(f"✓ {len(files)} arquivo(s) encontrado(s) na pasta '{pasta_experimento}':")
    for f in sorted(files):
        exp, rep, amo = extract_info(f)
        print(f"  - {os.path.basename(f)} -> Exp: {exp}, Rep: {rep}, Amostra: {amo}")

    return files


def juntar_cromatogramas(file_list):
    """
...
def processar_experimento(num_experimento, diretorio_base='.'):
    """
    Processa um experimento específico
    """
    print("=" * 60)
    print("PROCESSAMENTO DE CROMATOGRAMAS")
    print("=" * 60)
    print(f"\nExperimento selecionado: {num_experimento}")
    print(f"Procurando arquivos na pasta 'EXP {num_experimento}'...")
    print("─" * 60)

    # Buscar automaticamente todos os arquivos na pasta do experimento
    files = buscar_arquivos_cromatogramas(num_experimento, diretorio_base)
    ...
    if files:
        ...
        df_combined = juntar_cromatogramas(files)
        ...
        print(f"  2. Existem arquivos CSV no formato E{num_experimento}YZZ.csv dentro da pasta")


# ========== EXECUÇÃO ==========

# Para processar o experimento, defina o número e execute:
num_experimento = '4'  # ← ALTERE AQUI O NÚMERO DO EXPERIMENTO
processar_experimento(num_experimento)
